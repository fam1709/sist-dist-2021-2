...
def main():
	...
	while(run):
		...
		if option == 'E' or option == 'e':
			def callbackE3(ch, method, props, body):
				print(body.decode())
				channel.stop_consuming()
			channel.basic_publish(exchange='', routing_key='qm', body='E3['+'*e*[',properties=pika.BasicProperties(delivery_mode=2))
			channel.basic_consume(queue='E3', on_message_callback=callbackE3, auto_ack = True)
			channel.start_consuming()
			print('Para quem deseja enviar sua msng? Basta especificar seu numero')
			dest = input()
			print('Digite sua mensagem: ')
			msg = input()
			channel.basic_publish(exchange='', routing_key='qm', body='E3['+msg+'[E'+dest,properties=pika.BasicProperties(delivery_mode=2))
			print('Mensagem Enviada')
		if option == 'L' or option == 'l':
			method, prop, body = channel.basic_get(queue='E3')
			try:
